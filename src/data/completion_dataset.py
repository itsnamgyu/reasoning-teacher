...
class CompletionIdentifier:
    ...
    def __repr__(self):
        return "{}_{}_{}_{}_{}".format(self.base_model, self.completion_key, self.dataset_key,
                                       self.train_key or "NAN", self.epoch or "NAN")
...
class CompletionMetadata(CompletionIdentifier):
    ...
    def __init__(self, base_model: str, completion_key: str, dataset_key: str, finetune_key: str,
                 prediction_template: str, train_key: str = None, epoch: int = None):
...
class CompletionDataset:
    def __init__(self, raw_data: Dict):
        ...
        self.base_model = self.metadata["base_model"]
        self.dataset_key = self.metadata["dataset_key"]
        self.completion_key = self.metadata["completion_key"]
        self.finetune_key = self.metadata["finetune_key"]
        self.train_key = self.metadata["train_key"]
        self.epoch = self.metadata.get("epoch", None)
        self.prediction_template = self.metadata["prediction_template"]
