...
def cleanse_prediction(completion, dataset_key, answer_prefix=None, stop_sequence=None, return_all=True):
    if stop_sequence is not None:
        index = completion.find(stop_sequence)
        if index != -1:
...
def evaluate_completions(completion_data, dataset_key, template="special", stop_sequence="END", print_metrics=False,
    ...
    for index in indices:
        ...
        for s in selected_samples:
            ...
            if "finish_reason" in s:
                ...
                complete = s.get("finish_reason", None) == "stop" or stop_sequence in s["completion"]
